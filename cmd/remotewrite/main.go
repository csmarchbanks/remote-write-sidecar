...
func main() {
	...
	a := kingpin.New(filepath.Base(os.Args[0]), "The Remote Write Sidecar for Prometheus")

	a.Version(version.Print("remotewrite"))

	a.HelpFlag.Short('h')

	a.Flag("config.file", "Remote Write Sidecar configuration file path.").
		Default("remotewrite.yml").StringVar(&cfg.configFile)
	...
	remoteWriteStorage := remote.NewWriteStorage(logger, cfg.localStoragePath, time.Duration(cfg.remoteFlushDeadline))
